...
export interface Frame{tick:number;sourceLine?:number;robot:Robot;enemy:Enemy;variables:Record<string,RuntimeValue>;sensors:{name:string,value:boolean}[];action?:string;events:string[];error?:string}
...
export class ExpeditionRun{
  ...
  modifiers(metaUpgrades:string[]=[]):SimulationModifiers{const maxHp=this.maxHull()+(metaUpgrades.includes('hull1')?1:0);return{maxHp,maxEnergy:(this.hasReward('rewind')?30:20)+(metaUpgrades.includes('energy10')?10:0),attackPower:this.hasReward('echo')?2:1,moveEnergyCost:this.hasReward('dash')?0:1,incomingDamage:this.hasReward('shield')?0:1,startingHp:this.stats.nodesCleared?Math.min(this.hull,maxHp):undefined,energyRegenEvery:this.hasReward('regen')?5:undefined,rangedPower:metaUpgrades.includes('ranged1')?2:1}}
...
export class Simulation {pulseUsed=false;shieldReady=false;levelIndex=0;map=STORY_LEVELS[0].map;robot:Robot={x:1,y:1,dir:'E',hp:5,energy:20};enemy:Enemy={x:-1,y:-1,hp:0,kind:'slime'};items:Item[]=[];tick=0;frames:Frame[]=[];breakpoints=new Set<number>();watchpoints=new Set<string>();coreDump?:CoreDump;completedLevels=new Set<string>();profile:ProfileStats={ticks:0,actions:0,errors:0,durationMs:0,actionCounts:{}};private runStartedAt=0;private hitBreakpoints=new Set<number>();private watchedValues=new Map<string,RuntimeValue|undefined>();private scenario?:SimulationScenario;private modifiers:SimulationModifiers={maxHp:5,maxEnergy:20,attackPower:1,moveEnergyCost:1,incomingDamage:1};private seed=0;private rngState=1;private enemyJitter={attack:0,move:0};interpreter?:Interpreter;status:'idle'|'running'|'paused'|'success'|'failed'|'error'='idle';message='Ready';
 ...
 reset(){const l=this.scenario??STORY_LEVELS[this.levelIndex];this.map=[...l.map];this.robot={x:1,y:1,dir:'E',hp:this.modifiers.startingHp??this.modifiers.maxHp,energy:this.modifiers.maxEnergy};this.enemy={x:l.enemy?.x??-1,y:l.enemy?.y??-1,hp:l.enemy?.hp??0,kind:'slime'};this.items=this.scenario?.items?[...this.scenario.items]:[];this.enemyJitter=this.seed?{attack:this.nextInt(0,2),move:this.nextInt(0,2)}:{attack:0,move:0};this.tick=0;this.frames=[];this.pulseUsed=false;this.shieldReady=false;this.hitBreakpoints.clear();this.watchedValues.clear();this.coreDump=undefined;this.profile={ticks:0,actions:0,errors:0,durationMs:0,actionCounts:{}};this.runStartedAt=Date.now();this.status='running';this.message='Running'}
 ...
 private front(){const d={N:[0,-1],E:[1,0],S:[0,1],W:[-1,0]}[this.robot.dir];return{x:this.robot.x+d[0],y:this.robot.y+d[1]}} private isWall(x:number,y:number){return this.map[y]?.[x]==='#'} private resolveEnemy(){
    ...
    const attackEvery=(this.enemy.attackEvery??(kind==='swarm'?1:3))+this.enemyJitter.attack;
    ...
    if(inRange&&this.tick%attackEvery===0){
      ...
      if(kind==='tank')damage+=1;if(this.shieldReady){damage=Math.max(0,damage-1);this.shieldReady=false;}
      this.robot.hp=Math.max(0,this.robot.hp-damage);
      this.message=kind==='turret'?'敌方炮台射击':'Enemy strike';
    }
    if(kind==='turret')return;
    const moveEvery=(this.enemy.moveEvery??(kind==='swarm'?1:kind==='tank'?4:3))+this.enemyJitter.move;
