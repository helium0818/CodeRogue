...
export function expeditionPool(kind:'combat'|'elite'|'boss'):SimulationScenario[]{if(kind==='combat')return[...EXPEDITION_COMBAT_EXTRA,EXPEDITION_SCENARIOS.combat,COMBAT_VARIANT_B,COMBAT_VARIANT_C,COMBAT_VARIANT_D,COMBAT_VARIANT_E];if(kind==='elite')return[...EXPEDITION_ELITE_EXTRA,EXPEDITION_SCENARIOS.elite,ELITE_VARIANT_B,ELITE_VARIANT_C,ELITE_VARIANT_D];return[EXPEDITION_SCENARIOS.boss,BOSS_VARIANT_B]}export function pickScenario(kind:'combat'|'elite'|'boss',seed:number,index:number):SimulationScenario{const pool=expeditionPool(kind);return pool[(seed+index)%pool.length]}export const EXPEDITION_HUB_SCENARIO:SimulationScenario={id:'exp-hub',title:'远征中转站',objective:'选择路线行动，准备下一场代码战斗',map:['#########','#R.....E#','#########'],starterCode:'void update() {\n  wait();\n}',tactics:['事件与商店不需要运行固件。','你的选择会改变资源与下一场战斗的准备状态。']};
...
export class ExpeditionRun{
  ...
  private generateRoute(seed:number):ExpeditionNode[]{const state=(seed>>>0)||1;const rotate=state%2;return rotate===0?['combat','branch','combat','combat','shop','rest','event','boss']:['combat','branch','combat','combat','shop','event','rest','boss']}
