...
export interface SimulationSnapshot{levelIndex:number;map:string[];robot:Robot;enemy:Enemy;tick:number;frames:Frame[];status:Simulation['status'];message:string;interpreterGlobals:Record<string,RuntimeValue>;scenario?:SimulationScenario}
...
export class Simulation {pulseUsed=false;shieldReady=false;levelIndex=0;map=STORY_LEVELS[0].map;robot:Robot={x:1,y:1,dir:'E',hp:5,energy:20};enemy:Enemy={x:-1,y:-1,hp:0,kind:'slime'};tick=0;frames:Frame[]=[];breakpoints=new Set<number>();watchpoints=new Set<string>();coreDump?:CoreDump;completedLevels=new Set<string>();profile:ProfileStats={ticks:0,actions:0,errors:0,durationMs:0,actionCounts:{}};private runStartedAt=0;private hitBreakpoints=new Set<number>();private watchedValues=new Map<string,RuntimeValue|undefined>();private scenario?:SimulationScenario;private modifiers:SimulationModifiers={maxHp:5,maxEnergy:20,attackPower:1,moveEnergyCost:1,incomingDamage:1};interpreter?:Interpreter;status:'idle'|'running'|'paused'|'success'|'failed'|'error'='idle';message='Ready';
 ...
 snapshot():SimulationSnapshot{return{levelIndex:this.levelIndex,map:[...this.map],robot:{...this.robot},enemy:{...this.enemy},tick:this.tick,frames:this.frames.map(frame=>({...frame,robot:{...frame.robot},enemy:{...frame.enemy},variables:{...frame.variables},sensors:frame.sensors.map(sensor=>({...sensor}))})),status:this.status,message:this.message,interpreterGlobals:this.interpreter?{...this.interpreter.globals}:{},scenario:this.scenario?{...this.scenario,map:[...this.scenario.map],enemy:this.scenario.enemy?{...this.scenario.enemy}:undefined}:undefined}}
