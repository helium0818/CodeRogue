...
const enemyVisible=computed(()=>sim.enemy.hp>0&&Math.abs(sim.enemy.x-sim.robot.x)+Math.abs(sim.enemy.y-sim.robot.y)<=2);
...
function isVisible(x:number,y:number){return Math.abs(x-sim.robot.x)+Math.abs(y-sim.robot.y)<=2}
...
function completeExpeditionBattle(){if(gameMode.value!=='expedition'||!expeditionNeedsFirmware.value||expedition.nodeCleared)return;expedition.recordBattlePerformance(sim.robot.hp);expeditionBattleVerified.value=true;const damage=Math.max(0,maxHp.value-sim.robot.hp);const energyUsed=Math.max(0,maxEnergy.value-sim.robot.energy);const actions=sim.frames.filter(frame=>!!frame.action).length;const sensorReads=sim.frames.reduce((sum,frame)=>sum+frame.sensors.length,0);battleGrade.value=gradeBattle({tick:sim.tick,damage,energyUsed,actions,sensorReads});if(battleGrade.value==='S'||battleGrade.value==='A')selectedExpeditionAction.value='strike';else if(battleGrade.value==='B')selectedExpeditionAction.value='guard';else selectedExpeditionAction.value='hack';sim.message=`战斗固件通过 · ${battleGrade.value} 评级 · ${damage} 伤 · ${sim.tick} Tick · ${energyUsed} 能耗，请选择战术结算`}
...
function enterExpeditionScenario(loadExample=true){const scenario=expeditionScenario.value;if(!scenario)return;expeditionScenarioActive.value=true;expeditionBattleVerified.value=false;riskMode.value=false;const scaledScenario={...scenario,enemy:scenario.enemy?{...scenario.enemy,hp:scenario.enemy.hp+Math.floor(expedition.nodeIndex/4)}:undefined};sim.setScenario(scaledScenario,expedition.modifiers(meta.upgrades),expedition.seed);running.value=false;clearTimer();selected.value=0;buildError.value='';if(loadExample){code.value=scenario.starterCode;built.value=false;levelNotice.value=`已载入${scenario.title}地图与示例固件`;try{window.localStorage.setItem('coderogue.firmware-level','expedition')}catch{}}}
...
function resolveExpeditionAction(){const actionId=selectedExpeditionAction.value;if(!actionId){sim.message='请先选择一个行动，再确认执行';return}if(expeditionNeedsFirmware.value&&!expeditionBattleVerified.value){sim.message='请先在模拟舱运行固件并抵达出口，再执行遭遇战';return}if(expedition.resolveAction(actionId,expeditionBattleVerified.value||expeditionFirmwareReady.value,expeditionNeedsFirmware.value?battleGrade.value:undefined,expeditionNeedsFirmware.value?riskMode.value:false)){selectedExpeditionAction.value=undefined;expeditionBattleVerified.value=false;if(expeditionNode.value==='boss'){expedition.clearNode();summaryVisible.value=true}else rewardModalVisible.value=true}else if(expeditionNode.value==='shop'&&actionId==='buy'){sim.message=`资源不足：购买补给需要 ${expedition.shopBuyCost()} credits`} }
...
function settleExpeditionEscape(){expedition.clearNode();summaryVisible.value=true;resetExpeditionFirmware()}
...
function showStoryMode(){stop();rewardModalVisible.value=false;summaryVisible.value=false;riskMode.value=false;gameMode.value='story';expeditionScenarioActive.value=false;sim.selectLevel(sim.levelIndex);loadLevelExample(sim.levelIndex)}
...
let autoDemoTimer:number|undefined;
...
function autoAdvance(){if(!autoDemo.value)return;if(gameMode.value!=='expedition'){stopAutoDemo();return}if(expedition.nodeIndex>=expedition.route.length){stopAutoDemo();return}if(rewardModalVisible.value){chooseReward(expedition.choices()[0].id);confirmReward();return}if(summaryVisible.value){summaryVisible.value=false;return}if(expeditionNeedsFirmware.value){if(!expeditionScenarioActive.value){enterExpeditionScenario(true);return}if(expeditionBattleVerified.value&&!expedition.nodeCleared){resolveExpeditionAction();return}if(sim.status==='idle'||sim.status==='error'||sim.status==='failed'){loadReferenceSolution();build();run()}}else{if(!expedition.nodeCleared){if(!selectedExpeditionAction.value)chooseExpeditionAction(expedition.actions()[0].id);resolveExpeditionAction()}}}
function startAutoDemo(){stopAutoDemo();autoDemo.value=true;autoDemoTimer=window.setInterval(autoAdvance,900);autoAdvance()}
function newExpedition(){stop();rewardModalVisible.value=false;summaryVisible.value=false;riskMode.value=false;bankExpedition();const seedText=seedInput.value.trim();const parsed=Number(seedText);const seed=Number.isFinite(parsed)&&parsed>=0?Math.floor(parsed):Date.now();expedition.reset(seed);seedInput.value='';showExpeditionMode()}
function showExpeditionMode(){stop();rewardModalVisible.value=false;summaryVisible.value=false;riskMode.value=false;gameMode.value='expedition';if(expeditionNeedsFirmware.value)enterExpeditionScenario(true);else{expeditionScenarioActive.value=true;sim.setScenario(EXPEDITION_HUB_SCENARIO,expedition.modifiers(meta.upgrades),expedition.seed);sim.status='idle';sim.message='Route node loaded';built.value=false;code.value=EXPEDITION_HUB_SCENARIO.starterCode}}
showExpeditionMode();
...
<template>
  <main class="shell">
    ...
    <section v-if="gameMode==='expedition'" class="expedition-strip">
      ...
      <div v-else-if="expeditionNode && !expedition.nodeCleared && expeditionNeedsFirmware && expeditionBattleVerified" class="encounter-row battle-resolve"><div class="encounter-copy"><b>固件验证通过 · {{battleGrade}} 级</b><span>你已击破敌人并抵达撤离门。评级会决定收益，下面的策略是可选的覆盖选择。</span><small class="encounter-hint"><label class="risk-toggle"><input type="checkbox" v-model="riskMode"> 高风险加注：资源 +2、伤害 +1，但受伤 +1</label></small></div><div class="encounter-actions"><button v-for="action in expeditionActions" :key="action.id" :class="{selected:selectedExpeditionAction===action.id}" @click="chooseExpeditionAction(action.id)"><b>{{action.title}}</b><small>{{action.description}}</small></button><button class="confirm-reward" @click="resolveExpeditionAction" :disabled="!selectedExpeditionAction">确认战术</button></div></div>
      <div v-else-if="expeditionNode && !expedition.nodeCleared && expeditionNeedsFirmware" class="encounter-row battle-gate"><div class="encounter-copy"><b>{{expeditionNode==='boss'?'终局 BOSS':expeditionNode==='elite'?'精英战场':'遭遇战'}}</b><span>这是一场真正的代码战斗：地图上的机器人需要击败敌人并抵达出口。</span><small class="encounter-hint">草稿不完整，请在下方补全逻辑并运行固件</small></div><div class="encounter-actions"><button class="confirm-reward" @click="enterExpeditionScenario(true)" :disabled="expeditionScenarioActive">{{expeditionScenarioActive?'已在战场中':'进入战场'}}</button></div></div>
