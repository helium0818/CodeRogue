...
function finishTutorial(){sim.selectLevel(0);loadLevelExample(0);build();dismissTutorial()}
...
function completeExpeditionBattle(){if(gameMode.value!=='expedition'||!expeditionNeedsFirmware.value||expedition.nodeCleared)return;expedition.recordBattlePerformance(sim.robot.hp);expeditionBattleVerified.value=true;const damage=Math.max(0,maxHp.value-sim.robot.hp);battleGrade.value=damage===0&&sim.tick<=12?'S':damage<=1&&sim.tick<=18?'A':damage<=2?'B':'C';sim.message=`战斗固件通过 · ${battleGrade.value} 评级，请选择战术结算`}
