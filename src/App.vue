...
function enterExpeditionScenario(loadExample=true){const scenario=expeditionScenario.value;if(!scenario)return;expeditionScenarioActive.value=true;expeditionBattleVerified.value=false;riskMode.value=false;sim.setScenario(scenario,expedition.modifiers(meta.upgrades));running.value=false;clearTimer();selected.value=0;buildError.value='';if(loadExample){code.value=scenario.starterCode;built.value=false;levelNotice.value=`已载入${scenario.title}地图与示例固件`;try{window.localStorage.setItem('coderogue.firmware-level','expedition')}catch{}}}
