...
1. Dismiss the first-launch tutorial, choose any of the five freely selectable Story levels, and edit `update()` in the firmware editor.
...
The current release includes strict runtime validation, persistent typed state, bounded loops, fixed-size arrays, user functions, combat, deterministic debugging, Chinese responsive UI, tutorial guidance, optional audio feedback, local firmware/settings history, and a seven-node Expedition vertical slice.

## Verification

`npm test` and `npm run build` are the release checks. The five Story maps have automated solvability strategies and are regression-tested after runtime changes.

## UI Preview

![CodeRogue M9 interface](docs-m9-ui.png)
