...
The language parser/interpreter supports bounded `for` loops, fixed-size typed arrays, and user-defined functions with typed parameters and a 32-level call-depth guard. `Simulation.snapshot()` and `rollback()` clone world/trace/interpreter-global state. `ExpeditionRun` provides a seeded route, deterministic three-choice rewards, node progression, and settlement statistics for combat, elite, event, shop, rest, branch, and boss nodes.

`ExpeditionRun.resolveBattle()` settles combat directly from firmware grade and grants the mapped module or emergency resources; `resolveAction()` handles only event/shop/rest decisions; `chooseBranch()` rewrites the next node between event and elite. Rewards are tracked as a build and change simulation modifiers, while route node color coding, `lastOutcome`, and action logs keep results visible. `failDeployment()` makes robot destruction cost hull, and `scaleExpeditionBattle()` raises enemy HP/cadence deeper into the route.
