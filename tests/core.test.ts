...
import { EXPEDITION_HUB_SCENARIO, EXPEDITION_SCENARIOS, ExpeditionRun, LEVEL_STARTER_CODE, Simulation, STORY_LEVELS, STORY_PROGRESS_KEY, gradeBattle, gradeBattleReport, loadMeta, loadStoryBest, loadStoryProgress, pickScenario, recordStoryBest, saveMeta, saveStoryProgress } from '../src/core';
...
describe('simulation combat', () => {
  ...
  it('keeps every story level reachable with a firmware strategy', () => {
    const programs = [
      ...
      'void update(){ if(enemy_near()){ ranged_attack(); } else { move_forward(); } }',
      'bool defend_next=true; void update(){ if(enemy_ahead()){ if(defend_next){ shield(); } else { attack(); } defend_next=!defend_next; } else { move_forward(); } }'
    ];
  ...
  it('persists only improved story best grades and ticks', () => {
    const storage=new MemoryStorage();
    expect(recordStoryBest('0-1','A',10,storage)).toBe(true);
    expect(loadStoryBest(storage).bestGrades['0-1']).toBe('A');
    expect(recordStoryBest('0-1','B',5,storage)).toBe(false);
    expect(recordStoryBest('0-1','S',12,storage)).toBe(true);
    const best=loadStoryBest(storage);
    expect(best.bestGrades['0-1']).toBe('S');
    expect(best.bestTicks['0-1']).toBe(12);
    expect(recordStoryBest('unknown','A',1,storage)).toBe(false);
  });
  ...
  it('resolves a combat node directly from firmware grade and settles the boss', () => {
    const run=new ExpeditionRun(42);
    expect(run.nodeCleared).toBe(false);
    expect(run.resolveBattle(9,1,2,'S')).toBe(true);
    expect(run.nodeCleared).toBe(true);
    expect(run.log[0]).toContain('firmware-S');
    expect(['应急资源','扩容电池','声呐扩展']).toContain(run.lastOutcome.rewardTitle);
    run.clearNode();
    expect(run.nodeIndex).toBe(1);
    while(run.nodeIndex<run.route.length-1){
      if(['combat','elite','boss'].includes(run.current()!)){
        expect(run.resolveBattle(10,0,2,'A')).toBe(true);
      }else{
        if(run.current()==='branch'){expect(run.chooseBranch('safe')).toBe(true)}else{expect(run.resolveAction(run.actions()[0].id)).toBe(true)}
      }
      run.clearNode();
    }
    expect(run.current()).toBe('boss');
    expect(run.resolveBattle(12,1,5,'A')).toBe(true);
    run.clearNode();
    expect(run.stats.victory).toBe(true);
  });
  it('rejects an expedition action until the player explicitly chooses one', () => {
    const run=new ExpeditionRun(42);
    run.nodeIndex=run.route.indexOf('event');
    expect(run.resolveAction('')).toBe(false);
    expect(run.nodeCleared).toBe(false);
  });
  it('settles a combat node only once and rejects non-combat firmware settlement', () => {
    const run=new ExpeditionRun(42);
    expect(run.resolveBattle(9,0,2,'S')).toBe(true);
    expect(run.resolveBattle(9,0,2,'S')).toBe(false);
    run.nodeCleared=false;
    run.nodeIndex=run.route.indexOf('event');
    expect(run.resolveBattle(9,0,2,'S')).toBe(false);
  });
  ...
  it('maps S/A/B to safe auto rewards and C to emergency resources', () => {
    const safeIds=['rewind','sonar','longshot'];
    const s=new ExpeditionRun(42);
    expect(s.resolveBattle(9,0,2,'S')).toBe(true);
    if(s.lastOutcome.rewardId)expect(safeIds).toContain(s.lastOutcome.rewardId);
    expect(s.lastOutcome.credits).toBe(10);
    const a=new ExpeditionRun(42);
    expect(a.resolveBattle(9,0,2,'A')).toBe(true);
    if(a.lastOutcome.rewardId)expect(safeIds).toContain(a.lastOutcome.rewardId);
    expect(a.lastOutcome.credits).toBe(8);
    const b=new ExpeditionRun(42);
    expect(b.resolveBattle(9,0,2,'B')).toBe(true);
    if(b.lastOutcome.rewardId)expect(safeIds).toContain(b.lastOutcome.rewardId);
    expect(b.lastOutcome.credits).toBe(6);
    const c=new ExpeditionRun(42);
    expect(c.resolveBattle(9,0,2,'C')).toBe(true);
    expect(c.lastOutcome.rewardTitle).toBe('应急资源');
    expect(c.rewards).toHaveLength(0);
    expect(c.lastOutcome.credits).toBe(4);
  });
  it('carries expedition hull between nodes and lets a shop repair it', () => {
    const run=new ExpeditionRun(42);
    expect(run.resolveBattle(8,2,2,'C')).toBe(true);
    run.clearNode();
    expect(run.hull).toBe(3);
    while(run.current()!=='shop'){
      if(['combat','elite','boss'].includes(run.current()!)){
        expect(run.resolveBattle(8,0,2,'C')).toBe(true);
      }else{
        if(run.current()==='branch'){expect(run.chooseBranch('safe')).toBe(true)}else{expect(run.resolveAction(run.actions()[0].id)).toBe(true)}
      }
      run.clearNode({damageTaken:0,credits:1});
    }
    expect(run.resolveAction('buy')).toBe(true);
    run.clearNode();
    expect(run.hull).toBe(5);
  });
  it('lets a rest node repair hull without spending credits', () => {
    const run=new ExpeditionRun(42);
    run.nodeIndex=run.route.indexOf('rest');
    run.nodeCleared=false;
    run.hull=1;
    expect(run.resolveAction('repair')).toBe(true);
    run.clearNode();
    expect(run.hull).toBe(5);
  });
  it('does not allow buying supplies without enough credits', () => {
    ...
    while(run.current()!=='shop'){
      ...
      run.clearNode({damageTaken:0,credits:0});
    }
    expect(run.resolveAction('buy')).toBe(false);
    expect(run.nodeCleared).toBe(false);
  });
  it('carries firmware damage directly into battle settlement and hull', () => {
    const run=new ExpeditionRun(42);
    expect(run.resolveBattle(9,2,2,'A')).toBe(true);
    run.clearNode();
    expect(run.stats.damageTaken).toBe(2);
    expect(run.hull).toBe(3);
  });
  it('settles a boss firmware victory as a completed expedition', () => {
    const run=new ExpeditionRun(42);
    while(run.nodeIndex<run.route.length-1){
      if(['combat','elite','boss'].includes(run.current()!)){
        expect(run.resolveBattle(8,0,2,'C')).toBe(true);
    ...
    expect(run.current()).toBe('boss');
    expect(run.resolveBattle(10,1,5,'A')).toBe(true);
    run.clearNode();
    expect(run.stats.victory).toBe(true);
    expect(run.stats.nodesCleared).toBe(run.route.length);
  });
  it('costs hull when a combat deployment fails and ignores non-combat failures', () => {
    const run=new ExpeditionRun(42);
    expect(run.failDeployment()).toBe(true);
    expect(run.hull).toBe(4);
    expect(run.nodeCleared).toBe(false);
    expect(run.failDeployment()).toBe(true);
    expect(run.hull).toBe(3);
    run.nodeIndex=run.route.indexOf('event');
    run.nodeCleared=false;
    expect(run.failDeployment()).toBe(false);
    expect(run.hull).toBe(3);
  });
  it('applies purchased meta upgrades to expedition modifiers', () => {
    const run=new ExpeditionRun(42);
    const mods=run.modifiers(['hull1','energy10','ranged1']);
    expect(mods.maxHp).toBe(6);
    expect(mods.maxEnergy).toBe(30);
    expect(mods.rangedPower).toBe(2);
  });
  it('lets a branch node rewrite the next node between event and elite', () => {
    const safe=new ExpeditionRun(42);
    safe.nodeIndex=safe.route.indexOf('branch');
    safe.nodeCleared=false;
    expect(safe.chooseBranch('safe')).toBe(true);
    expect(safe.current()).toBe('event');
    const risk=new ExpeditionRun(42);
    risk.nodeIndex=risk.route.indexOf('branch');
    risk.nodeCleared=false;
    expect(risk.chooseBranch('risk')).toBe(true);
    expect(risk.current()).toBe('elite');
    expect(risk.chooseBranch('risk')).toBe(false);
  });
  ...
  it('grades battles by speed, damage, energy, actions and sensors', () => {
    ...
    const report=gradeBattleReport({tick:8,damage:0,energyUsed:10,actions:9,sensorReads:8});
    expect(report.grade).toBe('S');
    expect(report.reasons.join(' ')).toContain('无伤');
    expect(report.reasons.join(' ')).toContain('速度 8 Tick');
  });
  it('keeps generated expedition routes within bounds and structurally valid', () => {
    for(const seed of [1,2,42,2026,999999]){
      ...
      expect(run.route).toContain('shop');
      expect(run.route).toContain('event');
      expect(run.route).toContain('rest');
      expect(run.route).toContain('branch');
    }
  });
  ...
  it('persists recent expedition records in meta storage', () => {
    const storage=new MemoryStorage();
    saveMeta({credits:9,runs:1,upgrades:[],lastRuns:[{seed:42,result:'victory',nodesCleared:8,routeLength:8,credits:30,rewards:['rewind'],bestGrade:'A'}]},storage);
    const meta=loadMeta(storage);
    expect(meta.lastRuns?.[0]?.seed).toBe(42);
    expect(meta.lastRuns?.[0]?.result).toBe('victory');
    expect(meta.lastRuns?.[0]?.rewards).toEqual(['rewind']);
  });
  ...
  it('scales firmware rewards by battle grade', () => {
    const base=new ExpeditionRun(42);
    expect(base.resolveBattle(9,0,2,'C')).toBe(true);
    const graded=new ExpeditionRun(42);
    expect(graded.resolveBattle(9,0,2,'S')).toBe(true);
    expect(graded.lastOutcome.credits).toBe(base.lastOutcome.credits+6);

    expect(base.lastOutcome.rewardTitle).toBe('应急资源');
  });
  ...
  it('completes a full expedition run end to end once the meta grid is upgraded', () => {
    const run=new ExpeditionRun(42);
    const metaUpgrades=['energy10'];
    while(run.nodeIndex<run.route.length){
      ...
      if(['combat','elite','boss'].includes(node)){
        ...
        const sim=new Simulation();
        sim.setScenario(scenario, run.modifiers(metaUpgrades));
        ...
        const damage=Math.max(0,run.modifiers(metaUpgrades).maxHp-sim.robot.hp);
        const energyUsed=Math.max(0,run.modifiers(metaUpgrades).maxEnergy-sim.robot.energy);
